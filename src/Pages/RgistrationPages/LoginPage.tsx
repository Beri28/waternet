...
const LoginForm: React.FC<LoginFormProps> = () => {
  ...
  const handleSubmit =async (event: React.FormEvent) => {
    ...
    try {
      ...
      console.log(res)
      showToast("Login request success.", { type: 'success', theme: 'colored' })
      setLoading(false)
      navigate('/home')
    } catch (error:any) {
      console.log(error.response.data.message)
      showToast(` ${error.response.data.message}`, { type: 'error', theme: 'colored' })
      showToast(` Login failed`, { type: 'error', theme: 'colored' })
      setLoading(false)
    }
      ...
      <div className="bg-white p-8 rounded-lg shadow-md w-96">
        <div className="flex flex-col items-center mb-6">
          <div className="flex items-center">
            ...
            <h2 className="text-2xl font-semibold text-gray-800">NjanBiz</h2>
          </div>
          <p className="text-sm text-gray-600 mt-1">Welcome, Sign in to continue</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <p className="text-xs text-gray-600">
              Forgot Password? <a href="#" className="font-semibold text-indigo-600 hover:text-indigo-500">Reset password</a>
            </p>
          </div>
        ...
        <p className="mt-4 text-center text-sm text-gray-600">
          Don't have an account? <Link to="/register" className="font-semibold text-indigo-600 hover:text-indigo-500">Signup</Link>
        </p>
      </div>
    </div>
  );
